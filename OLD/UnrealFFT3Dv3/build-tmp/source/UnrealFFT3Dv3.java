...
public class UnrealFFT3Dv3 extends PApplet {
...
//*** AUDIO ***\\\
	...
	float band_max_abs = 80;							//Max value of all bands
	...
	float bass_max = 0;				//Max level reached  for bass
...
public void audio_bars(long t_now){
	...
	for(int i = 0; i < bars_size; i++) {
		...
		if(mode_3D){
			...
			for(int j = history_band_size-10; j>=0;j--){
...
class Zcam {
 ...
 public int camYmouse;
  ...
  float camOriginDumping=0.9f;
  float camOriginSpeed=0.005f;
  float camAngleDumping=0.9f;
  float camAngleSpeed=0.005f;
  ...
  Zcam()
   {
     ...
     temp=new PVector();
      ...
      camDistance=1000;
                 ...
                 camUp.y=1;
                 camUp.z=0;
       camOrigin.x=width/2.0f;
       camOrigin.x=height/2.0f;
       camOrigin.x=0;
...
public void mouseDragged() { 
          ...
          if (mouseButton == RIGHT) {
            myCamera.camAngleNext.y+=(-myCamera.camXmouse+mouseX)/100.0f;
            myCamera.camAngleNext.x+=(myCamera.camYmouse-mouseY)/100.0f;
          } 
...
class LFO {
    ...
    LFO(float per) { // constructor
          m = millis();
          period=per; }
    public float val() // return function
       {
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "--present", "--window-color=#666666", "--stop-color=#cccccc", "UnrealFFT3Dv3" };
    if (passedArgs != null) {
